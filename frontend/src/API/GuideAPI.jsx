...
export const TypeOfRefusalData = createAsyncThunk(
    "typeofrefusal/TypeOfRefusalData",
    async (id, { rejectWithValue }) => {
        try {
            const accessToken = localStorage.getItem("accessToken");
            const header = {
            ...
            if (id) {
                url = 'http://127.0.0.1:8000/api/v1/typeOfRefusal/' + id;
            }
            else
            {
                url = 'http://127.0.0.1:8000/api/v1/typeOfRefusal/'
            }
...
export const MethodOfRepairData = createAsyncThunk(
    "methodofrepair/MethodOfRepairData",
    async (id, { rejectWithValue }) => {
        try {
            const accessToken = localStorage.getItem("accessToken");
            const header = {
            ...
            let url = '';
            if (id) {
                url = "http://127.0.0.1:8000/api/v1/methodOfRepair/" + id;
            }
            else
            {
                url = "http://127.0.0.1:8000/api/v1/methodOfRepair/";
            }
            const { data } = await axios.get(
                url,
                header
...
const GuideAPI = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(TypeOfRefusalData.fulfilled, (state, action) => {
        ...
        builder.addCase(TypeOfRefusalData.pending, (state) => {
            state.loading = true;
            state.error = null;
        });

        builder.addCase(TypeOfRefusalData.rejected, (state, action) => {
            state.error = action.payload;
            state.loading = false;
            state.status = "BAD";
        });

        builder.addCase(MethodOfRepairData.fulfilled, (state, action) => {
            state.methodofrepair = action.payload;
            state.status = "OK";
            state.loading = false;
            state.success = true;
        });

        builder.addCase(MethodOfRepairData.pending, (state) => {
            state.loading = true;
            state.error = null;
        });

        builder.addCase(MethodOfRepairData.rejected, (state, action) => {
            state.error = action.payload;
            state.loading = false;
            state.status = "BAD";
        });
...
export const { guideClear } = GuideAPI.actions;
export default GuideAPI.reducer;
