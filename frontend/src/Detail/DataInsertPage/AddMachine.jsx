...
const AddMachine = () => {
    ...
    const guideList = useSelector((state) => state.guide);
    const created = useSelector((state) => state.machine);

    React.useEffect(() => {
        if (!guideList.machinelist) {
            dispatch(MachineListData());
        }
        if (!guideList.engine) {
            dispatch(EngineData());
        }
        if (!guideList.transmission) {
            dispatch(TransmissionData());
        }
        if (!guideList.mainaxle) {
            dispatch(MainAxleData());
        }
        if (!guideList.steeringaxle) {
        ...
        created.addmachine,
        navigate,
        guideList.machinelist,
        guideList.engine,
        guideList.transmission,
        guideList.mainaxle,
        guideList.steeringaxle,
        guideList.users,
    ]);
        ...
        <div>
            ...
            >
                ...
                <FormControl style={{ margin: "10px 0" }} required>
                    ...
                    >
                        {guideList.machinelist &&
                            guideList.machinelist.map((item) => (
                ...
                <FormControl style={{ margin: "10px 0" }} required>
                    ...
                    >
                        {guideList.transmission &&
                            guideList.transmission.map((item) => (
                                <MenuItem key={item.id} value={item.title}>
                                    {item.title}
                                </MenuItem>
                            ))}
                ...
                <FormControl style={{ margin: "10px 0" }} required>
                    ...
                    >
                        {guideList.steeringaxle &&
                            guideList.steeringaxle.map((item) => (
                                <MenuItem key={item.id} value={item.title}>
                                    {item.title}
                                </MenuItem>
                            ))}
