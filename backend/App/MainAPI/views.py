...
class MachineViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        elif user.users.character == 'CR':
            return Machine.objects.filter(client=user)
        elif user.users.character == 'SC':
...
class MaintenanceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_anonymous:
            return ''
        elif user.is_superuser or user.users.character == 'MR':
    ...
    def create(self, request, *args, **kwargs):
        if self.request.method == 'POST':
            ...
            newComplaints = {
                ...
                'dateOfRefusal': datetime.strptime(self.request.data['dateOfRefusal'], '%Y-%m-%d').date(),
                'operatingTime': self.request.data['operatingTime'],
                'nodeOfRefusal': TypeOfRefusal.objects.get(title = self.request.data['nodeOfRefusal']),
                'descriptionOfRefusal': self.request.data['descriptionOfRefusal'],
                'repairMethod': MethodOfRepair.objects.get(title = self.request.data['repairMethod']),
                'usedSpareParts': self.request.data['usedSpareParts'],
                'dateOfRepair': datetime.strptime(self.request.data['dateOfRepair'], '%Y-%m-%d').date(),
